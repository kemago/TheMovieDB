...
class MyForm(QWidget):
    ...
    def save_action(self):
        ...
        # ha létezik a json fájl, akkor megnyitjuk (ezt most nem használjuk fel később)
        if os.path.exists('datas.json'):
            with open('datas.json') as f:
                datas = json.load(f)

        # az ablakban beírt adatokból összeállítjuk az adatcsomagot
        datas[phone] = {
            'name': name,
            'address': address
        }
        # kiírjuk az adatokat a json fájl-ba
